...
void clearline(int r, int c, int len = 4)
{
	gotoRowCol(r, c);
	for (int i = 0;i < len;i++)
	{
...
void init(Piece* Board[][8], player PS[], Color& T)
{
	...
	for (int ri = 0; ri < 8; ri++)
	{
		...
		{
			Board[ri][ci] = new Piece{ '-',Black,{ri,ci},nullptr,nullptr};
		}
	}
	for (int ci = 0; ci < 8; ci++)
	{
		Board[1][ci] = new Piece{ 'P',White,{1,ci},isLegalPawn,pawn};
	}
	for (int ci = 0; ci < 8; ci++)
	{
		Board[6][ci] = new Piece{ 'p',Black,{6,ci},isLegalPawn,pawn };
	}
	//upper
	Board[0][0] = new Piece{ 'R',White,{0,0} ,isLegalRook,rook };
	Board[0][7] = new Piece{ 'R',White,{0,7},isLegalRook,rook };
	Board[0][1] = new Piece{ 'N',White,{0,1},isLegalKnight,knight };
	Board[0][6] = new Piece{ 'N',White,{0,6},isLegalKnight,knight };
	Board[0][2] = new Piece{ 'B',White,{0,2},isLegalBishop,bishop };
	Board[0][5] = new Piece{ 'B',White,{0,5},isLegalBishop,bishop };
	Board[0][4] = new Piece{ 'K',White,{0,4},isLegalKing,king };
	Board[0][3] = new Piece{ 'Q',White,{0,3} ,isLegalQueen,queen };
	//lower
	Board[7][0] = new Piece{ 'r',Black,{7,0} ,isLegalRook,rook };
	Board[7][7] = new Piece{ 'r',Black,{7,7} ,isLegalRook,rook };
	Board[7][1] = new Piece{ 'n',Black,{7,1},isLegalKnight,knight };
	Board[7][6] = new Piece{ 'n',Black,{7,6},isLegalKnight,knight };
	Board[7][2] = new Piece{ 'b',Black,{7,2},isLegalBishop,bishop };
	Board[7][5] = new Piece{ 'b',Black,{7,5},isLegalBishop,bishop };
	Board[7][4] = new Piece{ 'k',Black,{7,4},isLegalKing,king };
	Board[7][3] = new Piece{ 'q',Black,{7,3},isLegalQueen,queen };
}
...
void loadgame(Piece* Board[][8], player PS[], Color& T)
{
	...
	for (int i = 0;i < 8;i++)
	{
		for (int j = 0;j < 8;j++)
		{
			  ...
			  Board[i][j] = new Piece{ s,C,{i,j},nullptr,nullptr };
			  if (s == '-')
			  {
				  Board[i][j]->C=Black;
			  }
...
void updateboard(Piece* Board[][8], POSITION s, POSITION d)
{
	...
	Board[s.row][s.col] = new Piece{ '-',Black,{s.row,s.col},nullptr };
...
bool ischeck(Piece* board[8][8], Color turn)
{
	...
	for (int i = 0;i < 8;i++)
	{
		for (int j = 0;j < 8;j++)
		{
			s.row = i;
			s.col = j;
			if (isvalidsource(board, s, turn) and board[i][j]->C==turn)
			{ 
			if(board[i][j]->isvalidmove(s, D, board) and board[i][j]->isvalidmove)
			{
				return true;
			}
			}
...
bool isselfcheck(Piece* board[8][8],POSITION S,POSITION D, Color turn)
{
	...
	board[S.row][S.col]=new Piece{'-',Black,{S.row,S.col},nullptr,nullptr};
	bool check = ischeck(board,turn);
